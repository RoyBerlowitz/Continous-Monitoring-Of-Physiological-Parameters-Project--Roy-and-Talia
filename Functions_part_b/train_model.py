...
def choose_hyperparameters(train_df, labels, model=ModelNames.SVM, n_jobs = -1, n_iterations = 50, split_name = "Individual Split", split_by_group_flag = False):
    ...
    if model == ModelNames.SVM:
        best_SVM_parameters = find_best_SVM_parameters(df_for_hyperparameters, target,group_indicator, n_jobs, n_iterations = n_iterations, split_name = split_name, split_by_group_flag = split_by_group_flag)
...
def train_model(X_selected,y_train, best_parameters, model_name, split_by_group_flag = False):
    ...
    if model_name == ModelNames.LOGISTIC:
        return train_logistic_regression( train_x, Y_train,best_parameters, split_by_group_flag = split_by_group_flag)
    if model_name == ModelNames.XGBOOST:
        return train_xgboost(train_x, Y_train, best_parameters, split_by_group_flag = split_by_group_flag)
