...
def choose_hyperparameters(train_df, labels, model=ModelNames.SVM, n_jobs = -1, n_iterations = 50, split_name = "Individual Split", split_by_group_flag = False, wrapper_text = "", subsampling_flg = False):
    ...
    if model == ModelNames.RANDOM_FOREST:
        best_Random_Forrest_parameters = find_best_random_forrest_parameters(df_for_hyperparameters, target,group_indicator, n_jobs, n_iterations = n_iterations, split_name = split_name, split_by_group_flag = split_by_group_flag, wrapper_text=wrapper_text, subsampling_flg = subsampling_flg)
    ...
    if model == ModelNames.XGBOOST:
        return find_best_hp_xgboost(df_for_hyperparameters, target, split_name, split_by_group_flag, group_indicator, wrapper_text, subsampling_flg = subsampling_flg)

    return None

def train_model(X_selected,y_train, best_parameters, model_name, split_by_group_flag = False, subsampling_flg = False):
