...
def select_features(X, Y, function_variable: list, split_by_group_flag = False):
    ...
    selected_features = select_features_wrapper(X, Y, frozen_params,
                                    ...
                                    model_type=model_type, split_by_group_flag = split_by_group_flag, group_indicator=group_indicator)
    return selected_features

def select_features_wrapper(train_df, train_labels, frozen_params,
                                        n_features_range=[3, 5, 7, 10, 12, 15],
                                        model_type=WindowModelNames.RANDOM_FOREST, split_by_group_flag = False, group_indicator=None):
    ...
    if model_type == WindowModelNames.RANDOM_FOREST:
    ...
    elif model_type == WindowModelNames.XGBOOST:
        # We clean the parametrs' names if thet are with Pipeline prefix
        clean_params = {k.split('__')[-1]: v for k, v in frozen_params.items()}
    ...
    print(f"Starting RFECV (Automatic Feature Selection) for {model_type}...")
