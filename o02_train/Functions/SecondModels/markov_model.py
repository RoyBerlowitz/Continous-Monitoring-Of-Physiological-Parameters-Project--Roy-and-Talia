...
def prepare_data_for_hmm(seconds_df, target):
    ...
    # we avoid radical values to help converge to threshold other than binary
...
def calculate_real_transitions(y_train, lengths, alpha = 2.0):
    ...
    # we will use laplacian smoothing with the coefficient alpha for the smoothing
    ...
    p_01 = (off_to_on + alpha) / (total_0 + 2 * alpha)
    ...
    p_10 = (on_to_off+ alpha) / (max(total_1, 1) + 2*alpha)
    # transition from non-handwashing to non-handwashing
    p_00 = (total_0-off_to_on + alpha) / (total_0 + 2 * alpha)
    # transition from handwashing to handwashing
    p_11 = (total_1 -on_to_off+ alpha) / (max(total_1, 1) + 2*alpha)
...
def train_supervised_hmm(X_train, y_train, lengths):
    ...
    # the reason for the insecure_coeffienct addition is to make a threshold for the covariance which enhances stabillity and make the model less strict
    insecure_coeffienct = 0.01
    covariances = np.array([np.var(X_train[y_train == i], axis=0) + insecure_coeffienct for i in [0, 1]])
    ...
    model = hmm.GaussianHMM(n_components=2, covariance_type="diag", min_covar=0.01)
